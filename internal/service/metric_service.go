...
import (
	...
	"github.com/wybroot/alink/internal/metric"
...
func (s *MetricService) DeleteAgentMetrics(ctx context.Context, agentID string) error {
	...
	// 删除 VictoriaMetrics 中该探针的所有 alink 指标
	matcher := fmt.Sprintf(`{__name__=~"alink_.*",agent_id=%q}`, agentID)
...
func (s *MetricService) DeleteMonitorMetrics(ctx context.Context, monitorID string) error {
	...
	// 删除 VictoriaMetrics 中该监控任务的所有 alink monitor 指标
	matcher := fmt.Sprintf(`{__name__=~"alink_monitor_.*",monitor_id=%q}`, monitorID)
	if err := s.vmClient.DeleteSeries(ctx, []string{matcher}); err != nil {
...
func (s *MetricService) GetAvailableNetworkInterfaces(ctx context.Context, agentID string) ([]string, error) {
	...
	match := []string{fmt.Sprintf(`alink_network_sent_bytes_rate{agent_id="%s"}`, agentID)}
...
func (s *MetricService) buildPromQLQueries(agentID, metricType string, interfaceName string, aggregation string, step time.Duration) []metric.QueryDefinition {
	...
	case "cpu":
		queries = []metric.QueryDefinition{{
			Name:  "usage",
			Query: fmt.Sprintf(`alink_cpu_usage_percent{agent_id="%s"}`, agentID),
		}}

	case "memory":
		queries = []metric.QueryDefinition{{
			Name:  "usage",
			Query: fmt.Sprintf(`alink_memory_usage_percent{agent_id="%s"}`, agentID),
		}}

	case "disk":
		queries = []metric.QueryDefinition{{
			Name:  "usage",
			Query: fmt.Sprintf(`alink_disk_usage_percent{agent_id="%s",mount_point=""}`, agentID),
		}}
	...
	case "network":
		...
		if interfaceName != "" && interfaceName != "all" {
			...
			queries = []metric.QueryDefinition{
				{
					Name:   "upload",
					Query:  fmt.Sprintf(`alink_network_sent_bytes_rate{agent_id="%s",interface="%s"}`, agentID, interfaceName),
					Labels: map[string]string{"interface": interfaceName},
				},
				{
					Name:   "download",
					Query:  fmt.Sprintf(`alink_network_recv_bytes_rate{agent_id="%s",interface="%s"}`, agentID, interfaceName),
					Labels: map[string]string{"interface": interfaceName},
				},
			}
		} else {
	...
	case "network_connection":
		// 网络连接统计：多个状态
		queries = []metric.QueryDefinition{
			{Name: "established", Query: fmt.Sprintf(`alink_network_conn_established{agent_id="%s"}`, agentID)},
			{Name: "time_wait", Query: fmt.Sprintf(`alink_network_conn_time_wait{agent_id="%s"}`, agentID)},
			{Name: "close_wait", Query: fmt.Sprintf(`alink_network_conn_close_wait{agent_id="%s"}`, agentID)},
			{Name: "listen", Query: fmt.Sprintf(`alink_network_conn_listen{agent_id="%s"}`, agentID)},
		}

	case "disk_io":
		// 磁盘 IO：读和写
		queries = []metric.QueryDefinition{
			{Name: "read", Query: fmt.Sprintf(`alink_disk_read_bytes_rate{agent_id="%s"}`, agentID)},
			{Name: "write", Query: fmt.Sprintf(`alink_disk_write_bytes_rate{agent_id="%s"}`, agentID)},
		}
	...
	case "gpu":
		...
		queries = []metric.QueryDefinition{
			{
				Name:  "utilization",
				Query: fmt.Sprintf(`alink_gpu_utilization_percent{agent_id="%s"}`, agentID),
			},
			{
				Name:  "temperature",
				Query: fmt.Sprintf(`alink_gpu_temperature_celsius{agent_id="%s"}`, agentID),
			},
		}
	...
	case "temperature":
		...
		queries = []metric.QueryDefinition{{
			Name:  "temperature",
			Query: fmt.Sprintf(`alink_temperature_celsius{agent_id="%s"}`, agentID),
		}}
	...
	case "monitor":
		...
		queries = []metric.QueryDefinition{{
			Name:  "response_time",
			Query: fmt.Sprintf(`alink_monitor_response_time_ms{agent_id="%s"}`, agentID),
		}}
	}
...
func (s *MetricService) buildMonitorPromQLQueries(monitorID string, aggregation string, step time.Duration) []metric.QueryDefinition {
	queries := []metric.QueryDefinition{
		{Name: "response_time", Query: fmt.Sprintf(`alink_monitor_response_time_ms{monitor_id="%s"}`, monitorID)},
	}
	return expandAggregationQueries(queries, aggregation, step)
}
