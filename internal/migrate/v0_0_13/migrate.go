...
func convertToMetrics(agentID string, metricType string, data interface{}, timestamp int64) []vmclient.Metric {
	...
	case protocol.MetricTypeCPU:
		...
		metrics = append(metrics, createMetric("alink_cpu_usage_percent", agentID, nil, cpuData.UsagePercent, timestamp))
		metrics = append(metrics, createMetric("alink_cpu_cores_logical", agentID, nil, float64(cpuData.LogicalCores), timestamp))
		metrics = append(metrics, createMetric("alink_cpu_cores_physical", agentID, nil, float64(cpuData.PhysicalCores), timestamp))

	case protocol.MetricTypeMemory:
		memData := data.(*protocol.MemoryData)
		metrics = append(metrics, createMetric("alink_memory_usage_percent", agentID, nil, memData.UsagePercent, timestamp))
		metrics = append(metrics, createMetric("alink_memory_total_bytes", agentID, nil, float64(memData.Total), timestamp))
		metrics = append(metrics, createMetric("alink_memory_used_bytes", agentID, nil, float64(memData.Used), timestamp))
		metrics = append(metrics, createMetric("alink_memory_available_bytes", agentID, nil, float64(memData.Available), timestamp))
		metrics = append(metrics, createMetric("alink_memory_swap_total_bytes", agentID, nil, float64(memData.SwapTotal), timestamp))
		metrics = append(metrics, createMetric("alink_memory_swap_used_bytes", agentID, nil, float64(memData.SwapUsed), timestamp))
	...
	case protocol.MetricTypeDisk:
		...
		for _, diskData := range diskDataList {
			...
			metrics = append(metrics, createMetric("alink_disk_usage_percent", agentID, labels, diskData.UsagePercent, timestamp))
			metrics = append(metrics, createMetric("alink_disk_total_bytes", agentID, labels, float64(diskData.Total), timestamp))
			metrics = append(metrics, createMetric("alink_disk_used_bytes", agentID, labels, float64(diskData.Used), timestamp))
			metrics = append(metrics, createMetric("alink_disk_free_bytes", agentID, labels, float64(diskData.Free), timestamp))
		}
	...
	case protocol.MetricTypeNetwork:
		...
		for _, netData := range networkDataList {
			...
			metrics = append(metrics, createMetric("alink_network_sent_bytes_rate", agentID, labels, float64(netData.BytesSentRate), timestamp))
			metrics = append(metrics, createMetric("alink_network_recv_bytes_rate", agentID, labels, float64(netData.BytesRecvRate), timestamp))
			metrics = append(metrics, createMetric("alink_network_sent_bytes_total", agentID, labels, float64(netData.BytesSentTotal), timestamp))
			metrics = append(metrics, createMetric("alink_network_recv_bytes_total", agentID, labels, float64(netData.BytesRecvTotal), timestamp))
		}

	case protocol.MetricTypeNetworkConnection:
		connData := data.(*protocol.NetworkConnectionData)
		metrics = append(metrics, createMetric("alink_network_conn_established", agentID, nil, float64(connData.Established), timestamp))
		metrics = append(metrics, createMetric("alink_network_conn_syn_sent", agentID, nil, float64(connData.SynSent), timestamp))
		metrics = append(metrics, createMetric("alink_network_conn_syn_recv", agentID, nil, float64(connData.SynRecv), timestamp))
		metrics = append(metrics, createMetric("alink_network_conn_time_wait", agentID, nil, float64(connData.TimeWait), timestamp))
		metrics = append(metrics, createMetric("alink_network_conn_close_wait", agentID, nil, float64(connData.CloseWait), timestamp))
		metrics = append(metrics, createMetric("alink_network_conn_listen", agentID, nil, float64(connData.Listen), timestamp))
		metrics = append(metrics, createMetric("alink_network_conn_total", agentID, nil, float64(connData.Total), timestamp))
	...
	case protocol.MetricTypeDiskIO:
		...
		metrics = append(metrics, createMetric("alink_disk_read_bytes_rate", agentID, nil, float64(totalReadRate), timestamp))
		metrics = append(metrics, createMetric("alink_disk_write_bytes_rate", agentID, nil, float64(totalWriteRate), timestamp))
	...
	case protocol.MetricTypeGPU:
		...
		for _, gpuData := range gpuDataList {
			...
			metrics = append(metrics, createMetric("alink_gpu_utilization_percent", agentID, labels, gpuData.Utilization, timestamp))
			metrics = append(metrics, createMetric("alink_gpu_memory_total_bytes", agentID, labels, float64(gpuData.MemoryTotal), timestamp))
			metrics = append(metrics, createMetric("alink_gpu_memory_used_bytes", agentID, labels, float64(gpuData.MemoryUsed), timestamp))
			metrics = append(metrics, createMetric("alink_gpu_temperature_celsius", agentID, labels, gpuData.Temperature, timestamp))
			metrics = append(metrics, createMetric("alink_gpu_power_draw_watts", agentID, labels, gpuData.PowerUsage, timestamp))
		}
	...
	case protocol.MetricTypeTemperature:
		...
		for _, tempData := range tempDataList {
			...
			metrics = append(metrics, createMetric("alink_temperature_celsius", agentID, labels, tempData.Temperature, timestamp))
		}
	...
	case protocol.MetricTypeMonitor:
		...
		for _, monitorData := range monitorDataList {
			...
			metrics = append(metrics, createMetric("alink_monitor_response_time_ms", agentID, labels, float64(monitorData.ResponseTime), timestamp))
		}
	}
